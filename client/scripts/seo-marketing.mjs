  ...
  <div class="lmsgen-seo-inner">
    ...
    <div class="lmsgen-pdf-course-proof publica-demo-shell" data-publica-demo>
      ...
      <div class="publica-reader-stage">
        <button type="button" class="publica-turn-control publica-prev" aria-label="Previous comic page">‹</button>
        <div class="publica-comic-book" aria-live="polite">
          <article class="publica-comic-page publica-page-left" data-publica-left></article>
          <article class="publica-comic-page publica-page-right" data-publica-right></article>
          <div class="publica-book-spine" aria-hidden="true"></div>
          <div class="publica-turn-sheet" aria-hidden="true"></div>
        </div>
        <button type="button" class="publica-turn-control publica-next" aria-label="Next comic page">›</button>
      </div>
...
<script id="lmsgen-publica-demo-script">(function(){document.querySelectorAll('[data-publica-demo]').forEach(function(demo){if(demo.dataset.ready)return;demo.dataset.ready='1';var pages=Array.from(demo.querySelectorAll('template[data-publica-page]')).map(function(item){return item.innerHTML});var left=demo.querySelector('[data-publica-left]');var right=demo.querySelector('[data-publica-right]');var status=demo.querySelector('[data-publica-status]');var prev=demo.querySelector('.publica-prev');var next=demo.querySelector('.publica-next');var book=demo.querySelector('.publica-comic-book');var mobile=window.matchMedia('(max-width:640px)');var index=0;var busy=false;function count(){return mobile.matches?1:2}function draw(){var visible=count();var end=Math.min(pages.length,index+visible);left.innerHTML=pages[index]||'';right.innerHTML=pages[index+1]||'';right.hidden=visible===1;status.textContent=(visible===1?'Page '+(index+1):'Pages '+(index+1)+'–'+end)+' of '+pages.length;prev.disabled=index===0;next.disabled=end>=pages.length;Array.from(demo.querySelectorAll('.publica-reader-dots i')).forEach(function(dot,dotIndex){dot.classList.toggle('is-active',dotIndex===Math.floor(index/2))})}function turn(direction){if(busy)return;var visible=count();var target=Math.max(0,Math.min(pages.length-visible,index+(direction>0?visible:-visible)));if(target===index)return;busy=true;book.classList.add(direction>0?'is-turning-next':'is-turning-prev');window.setTimeout(function(){index=target;draw()},220);window.setTimeout(function(){book.classList.remove('is-turning-next','is-turning-prev');busy=false},520)}prev.addEventListener('click',function(){turn(-1)});next.addEventListener('click',function(){turn(1)});demo.addEventListener('keydown',function(event){if(event.key==='ArrowLeft')turn(-1);if(event.key==='ArrowRight')turn(1)});if(mobile.addEventListener)mobile.addEventListener('change',function(){index=Math.min(index,pages.length-count());draw()});draw()})})();</script>`;
...
const PDF_COURSE_STYLE = `<style id="lmsgen-pdf-course-style">
  ...
  .publica-demo-shell{display:block!important;padding:1.8rem!important;background:#eaf8f6!important;overflow:hidden}
  .publica-demo-topbar{min-height:5.6rem;display:flex;align-items:center;justify-content:space-between;gap:2rem;padding:0 1rem 1.6rem;color:#003f3a}
  .publica-demo-topbar>div{display:flex;align-items:center;gap:1.1rem}.publica-demo-topbar strong{font-family:"Montserrat",Arial,sans-serif!important;font-size:1.55rem}.publica-demo-topbar small{font-size:1.1rem;color:#52726e}
  .publica-demo-mark{width:3.5rem;height:3.5rem;display:grid;place-items:center;border-radius:1rem;background:#0a5d56;color:#8af0e5;font-family:"Montserrat",Arial,sans-serif;font-size:1.5rem;font-weight:800}
  ...
  .publica-reader-stage{position:relative;min-height:0;display:grid;grid-template-columns:5rem minmax(0,1fr) 5rem;align-items:center;gap:1.5rem;padding:2.5rem;border-radius:2.2rem;background:radial-gradient(circle at 50% 40%,#174e55 0,#082f33 58%,#041d21 100%);overflow:hidden;perspective:180rem}
  ...
  .publica-comic-book{position:relative;z-index:1;width:min(100%,104rem);margin:auto;aspect-ratio:1.55;display:grid;grid-template-columns:1fr 1fr;transform-style:preserve-3d;filter:drop-shadow(0 2.8rem 2.5rem rgba(0,0,0,.42))}
  .publica-comic-page{position:relative;min-width:0;height:100%;overflow:hidden;background:#f8f1dd;color:#092f31;border:1px solid rgba(0,0,0,.12)}.publica-page-left{border-radius:1.2rem 0 0 1.2rem}.publica-page-right{border-radius:0 1.2rem 1.2rem 0}
  .publica-book-spine{position:absolute;z-index:4;left:50%;top:0;bottom:0;width:2.2rem;transform:translateX(-50%);background:linear-gradient(90deg,rgba(0,0,0,.24),rgba(255,255,255,.15) 45%,rgba(0,0,0,.22));mix-blend-mode:multiply;pointer-events:none}
  .publica-turn-sheet{position:absolute;z-index:5;top:0;right:0;width:50%;height:100%;border-radius:0 1.2rem 1.2rem 0;background:linear-gradient(90deg,#fff7dd,#e4d7b7);opacity:0;transform-origin:left center;backface-visibility:hidden;pointer-events:none}
  .publica-comic-book.is-turning-next .publica-turn-sheet{animation:publicaTurnNext .52s cubic-bezier(.35,.05,.2,1)}.publica-comic-book.is-turning-prev .publica-turn-sheet{left:0;right:auto;transform-origin:right center;animation:publicaTurnPrev .52s cubic-bezier(.35,.05,.2,1)}
  @keyframes publicaTurnNext{0%{opacity:1;transform:rotateY(0);box-shadow:-1rem 0 2rem rgba(0,0,0,.16)}49%{opacity:1}50%{opacity:.7}100%{opacity:0;transform:rotateY(-180deg);box-shadow:-4rem 0 5rem rgba(0,0,0,.28)}}
  @keyframes publicaTurnPrev{0%{opacity:1;transform:rotateY(0);box-shadow:1rem 0 2rem rgba(0,0,0,.16)}100%{opacity:0;transform:rotateY(180deg);box-shadow:4rem 0 5rem rgba(0,0,0,.28)}}
  .publica-turn-control{position:relative;z-index:6;width:4.7rem;height:4.7rem;border:1px solid rgba(137,239,229,.35);border-radius:50%;background:rgba(4,29,33,.72);color:#8af0e5;font:400 3.2rem/1 Arial;cursor:pointer;transition:transform .2s ease,background .2s ease}.publica-turn-control:hover:not(:disabled){transform:scale(1.08);background:#0b635b}.publica-turn-control:disabled{opacity:.22;cursor:default}
  ...
  @media(max-width:760px){.publica-reader-stage{grid-template-columns:4rem minmax(0,1fr) 4rem;gap:.7rem;padding:1.5rem}.publica-turn-control{width:3.8rem;height:3.8rem;font-size:2.5rem}.publica-demo-topbar small{display:none}}
  @media(max-width:640px){.lmsgen-pdf-course-header{gap:2.5rem}.lmsgen-pdf-course-benefits{margin-top:3.5rem}.lmsgen-pdf-course-benefits li{padding:2.2rem}.lmsgen-pdf-course-proof{padding:1rem;gap:1.5rem}.lmsgen-pdf-proof-screen{min-height:29rem;grid-template-columns:7.5rem 1fr}.lmsgen-pdf-proof-rail{padding:1.3rem .8rem}.lmsgen-pdf-proof-rail strong{font-size:.9rem}.lmsgen-pdf-proof-rail span{font-size:.75rem}.lmsgen-pdf-proof-slide{padding:.8rem}.lmsgen-pdf-proof-toolbar{font-size:.75rem}.lmsgen-pdf-proof-page{min-height:20rem;padding:1.8rem}.lmsgen-pdf-proof-page strong{font-size:2.7rem}.lmsgen-pdf-proof-page p{font-size:.9rem}.lmsgen-pdf-proof-copy{padding:1.5rem}.lmsgen-pdf-proof-copy h3{font-size:2.5rem}.publica-demo-shell{padding:1rem!important}.publica-demo-topbar{padding:.2rem .2rem 1rem}.publica-demo-topbar strong{font-size:1.2rem}.publica-demo-live{padding:.6rem .8rem;font-size:.8rem}.publica-reader-stage{grid-template-columns:1fr;padding:1.2rem 3.8rem}.publica-comic-book{width:min(100%,43rem);aspect-ratio:.72;grid-template-columns:1fr}.publica-page-left{border-radius:1rem}.publica-page-right,.publica-book-spine{display:none!important}.publica-turn-sheet{width:100%;border-radius:1rem}.publica-turn-control{position:absolute;top:50%;transform:translateY(-50%);width:3.6rem;height:3.6rem}.publica-turn-control:hover:not(:disabled){transform:translateY(-50%) scale(1.05)}.publica-prev{left:.4rem}.publica-next{right:.4rem}.publica-reader-footer{grid-template-columns:1fr auto;padding:1rem .3rem .2rem}.publica-reader-footer>strong{display:none}.publica-reader-dots{justify-self:end}.publica-panel-grid{gap:.8%;padding:1.5%}.publica-panel{border-width:.22rem}}
  @media(prefers-reduced-motion:reduce){.publica-comic-book.is-turning-next .publica-turn-sheet,.publica-comic-book.is-turning-prev .publica-turn-sheet{animation:none}}
</style>`;
