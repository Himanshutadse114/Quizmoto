...
const PDF_COURSE_STYLE = `<style id="lmsgen-pdf-course-style">
  ...
  .publica-reader-stage{position:relative;min-height:0;display:grid;grid-template-columns:5rem minmax(0,1fr) 5rem;grid-template-areas:"prev book next";align-items:center;gap:1.5rem;padding:2.5rem;border-radius:2.2rem;background:radial-gradient(circle at 50% 40%,#174e55 0,#082f33 58%,#041d21 100%);overflow:hidden;perspective:180rem}
  ...
  .publica-turn-control{position:relative;z-index:6;width:4.7rem;height:4.7rem;border:1px solid rgba(137,239,229,.35);border-radius:50%;background:rgba(4,29,33,.78);color:#8af0e5;display:inline-flex;align-items:center;justify-content:center;gap:.6rem;cursor:pointer;transition:transform .2s ease,background .2s ease,border-color .2s ease}.publica-turn-control span{font:400 3.2rem/1 Arial}.publica-turn-control b{display:none;font:750 1rem/1 "Montserrat",Arial,sans-serif}.publica-prev{grid-area:prev}.publica-next{grid-area:next}.publica-turn-control:hover:not(:disabled){transform:scale(1.08);background:#0b635b;border-color:#8af0e5}.publica-turn-control:disabled{opacity:.22;cursor:default}
