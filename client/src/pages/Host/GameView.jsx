                ...
                {(gameState === 'question' || gameState === 'result') && players.length > 0 && (
                    <div className="w-full max-w-6xl mt-8">
                        <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
                                    ...
                                    {[
                                        ...
                                        <button
                                            ...
                                            className={'flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ' + (
                                                presenceTab === id
                                                    ? 'bg-white text-quizmoto-purple border-white'
                                                    : 'bg-white/5 text-white/50 border-white/10 hover:text-white'
                                            )}
                                        >
                                            <Icon size={12} />
                                            {label}
                                            <span className={'min-w-[1.25rem] text-center rounded-full px-1 ' + (presenceTab === id ? 'bg-quizmoto-purple/15' : 'bg-white/10')}>
                                                {count}
                                            </span>
                                        </button>
                                    ))}
                                    ...
                                    <div
                                        ...
                                        className={'flex items-center gap-2 px-3 py-2 rounded-xl border text-sm ' + (
                                            p.socketId
                                                ...
                                                : 'bg-red-500/5 border-red-500/20 opacity-60 grayscale'
                                        )}
                                    >
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <p className={'text-[8px] font-black uppercase tracking-widest ' + (p.socketId ? 'text-emerald-400' : 'text-red-400')}>
                                                {p.socketId ? 'Active' : 'Offline'}
                                            </p>
                                        </div>
                            ...
                            {(presenceTab === 'active' ? players.filter(p => p.socketId) : presenceTab === 'offline' ? players.filter(p => !p.socketId) : players).length === 0 && (
                                <p className="text-center text-xs font-bold text-white/30 py-6 uppercase tracking-widest">
                                    {presenceTab === 'offline' ? 'No offline players' : 'No players in this view'}
                                </p>
                            )}
                        </div>
                    </div>
