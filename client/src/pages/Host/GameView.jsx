...
const GameView = () => {
    ...
    useEffect(() => {
        ...
        socket.on('room_info', (sessionData) => {
            ...
            if (sessionData.status === 'question' || sessionData.status === 'result') {
                ...
                // If we land here and state is lobby, it means the start_question event 
                // hasn't fully propagated yet, or something refreshed. We should trigger it.
        ...
        socket.on('game_finished', (data) => {
            setGameState('finished');
            setIsProcessingNext(false); // Reset so Loading... doesn't stay stuck
            // Support both old (array) and new (object) data formats
    ...
    const nextAction = () => {
        if (isProcessingNext) return; // Guard against double-click
        if (gameState === 'result') {
            // Single click: go directly to the next question or end the game
