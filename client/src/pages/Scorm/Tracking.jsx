...
import { Activity, Search, CheckCircle2, Clock3, CircleDashed, Users, MapPin, ExternalLink, ChevronDown, ChevronUp } from 'lucide-react';
          ...
          <table className="w-full min-w-[1080px] text-sm">
            <thead>
              <tr className="text-left scorm-micro text-[9px] uppercase font-semibold text-[#667085] border-b border-black bg-[#F8FAFC]">
                <th className="px-5 py-3.5 min-w-[230px]">Learner</th>
                <th className="px-5 py-3.5 min-w-[220px]">Course</th>
                <th className="px-5 py-3.5">Attempts</th>
                <th className="px-5 py-3.5 min-w-[190px]">Completion</th>
                <th className="px-5 py-3.5 min-w-[150px]">Last location</th>
                <th className="px-5 py-3.5">Score</th>
                <th className="px-5 py-3.5">Time</th>
                <th className="px-5 py-3.5 min-w-[135px]">Last activity</th>
                ...
                return (
                  <React.Fragment key={row.id}>
                    <tr
                      ...
                      className={`border-b border-[#E4E7EC] bg-white hover:bg-[#FAFAFF] cursor-pointer ${isExpanded ? 'bg-[#F7F8FF]' : ''}`}
                    >
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="w-10 h-10 rounded-xl border border-[#D0D5DD] bg-[#EEF4FF] text-[#3538CD] grid place-items-center font-black text-xs shrink-0">
                            {initials(row.learnerName, row.learnerEmail)}
                          </div>
                          <div className="min-w-0">
                            <div className="text-[8px] uppercase tracking-[0.1em] font-semibold text-[#98A2B3]">Learner name</div>
                            <div className="font-semibold text-black truncate">{row.learnerName || 'Learner'}</div>
                            <div className="text-[10px] text-[#667085] mt-0.5 truncate max-w-[190px]">{row.learnerEmail || 'No email'}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-4">
                        ...
                        <Link
                          to={`/scorm/courses/${row.courseId}`}
                          onClick={(event) => event.stopPropagation()}
                          className="inline-flex items-center gap-1 mt-1 text-[10px] font-semibold text-[#635BFF] hover:text-[#5145CD]"
                        >Open course <ExternalLink size={10} /></Link>
                      </td>
                      <td className="px-5 py-4">
                        <span className="inline-flex items-center gap-2 rounded-lg border border-[#D0D5DD] bg-[#F8FAFC] px-2.5 py-1.5 text-[#344054]">
                          ...
                          <strong className="text-xs text-[#101828]">{Math.max(1, Number(row.attemptCount || 1))}</strong>
                        </span>
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-between gap-3 mb-2">
                          <span className="text-[12px] font-semibold text-black">{row.progressAvailable ? `${Number(row.progressPercent).toFixed(0)}%` : '—'}</span>
                          <span className="rounded-full border border-[#D0D5DD] bg-[#F8FAFC] px-2 py-1 text-[8px] uppercase tracking-[0.08em] font-semibold text-[#667085]">{statusLabel(row)}</span>
                        </div>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-start gap-2 max-w-[180px]">
                          ...
                          <span className="text-[11px] font-medium text-[#667085] break-words">{row.lastLocation || 'Not started'}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4 text-right">
                        <button
                          type="button"
                          onClick={(event) => { event.stopPropagation(); setExpandedRowId(isExpanded ? null : row.id); }}
                          className="inline-flex items-center gap-1.5 rounded-lg border border-[#D0D5DD] bg-white px-2.5 py-1.5 text-[10px] font-semibold text-[#344054] hover:bg-[#F9FAFB]"
