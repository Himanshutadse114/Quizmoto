  ...
  const runGenerate = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        ...
        { analysis: clean, templateId, ...(replacePackageId ? { replacePackageId } : {}) },
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 relative z-10 max-w-4xl mx-auto pb-24">
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl md:text-3xl font-black italic tracking-tighter">{replacePackageId ? 'Edit course' : 'Create from policy'}</h1>
      </div>
      ...
      {editLoading && (
        <div className="rounded-3xl bg-white/5 border border-white/10 p-8 text-center text-white/70 font-bold">
          Loading package for edit…
        </div>
      )}

      {!editLoading && step === 'upload' && (
      ...
      {!editLoading && step === 'preview' && analysis && (
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sticky bottom-2 z-20">
            ...
            >
              {busy ? 'Building SCORM package…' : (replacePackageId ? 'Save edits to package' : 'Generate SCORM & save to library')}
            </button>
          </div>
        </div>
      )}

      {!editLoading && step === 'done' && resultPkg && (
