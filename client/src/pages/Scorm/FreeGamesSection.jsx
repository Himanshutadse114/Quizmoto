...
import { Gamepad2, LockKeyhole, Play, Shapes, Sparkles, X } from 'lucide-react';
...
export default function FreeGamesSection() {
  ...
  useEffect(() => {
    const handleMessage = (event) => {
      ...
      if (event.data?.type === 'lmsgen:geometry-close') setGameOpen(false);
    };
    ...
    <>
      <section className="scorm-panel overflow-hidden mb-6">
        <div className="scorm-panel-header flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          <div>
            <div className="scorm-eyebrow inline-flex items-center gap-2"><Gamepad2 size={13} /> Free Games</div>
            <h3 className="text-[18px] mt-1">Learn by playing</h3>
            <p className="scorm-meta mt-1.5">Interactive learning games run inside LMSGEN. Start free and ask the Super Admin when your tenant needs the full curriculum.</p>
          </div>
      ...
      {gameOpen && (
        <section className="fixed left-0 lg:left-[268px] right-0 top-[64px] bottom-0 z-[29] bg-[var(--scorm-bg,#F4F8F7)] flex flex-col" aria-label="Geometry Physics game">
          <div className="h-14 shrink-0 border-b border-[var(--scorm-border,#D7E5E2)] bg-[var(--scorm-panel,#fff)] px-4 md:px-5 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-8 h-8 rounded-lg grid place-items-center border border-[#4FC9BF]/30 bg-[#4FC9BF]/10 text-[#238E85] shrink-0"><Shapes size={16} /></div>
              <div className="min-w-0">
                <div className="text-xs font-semibold truncate">Geometry Physics</div>
                <div className="text-[9px] opacity-55 truncate">LMSGEN Free Games · {fullAccess ? `All ${TOTAL_LEVELS} levels unlocked` : `Levels 1–${FREE_LEVELS} free`}</div>
              </div>
            </div>
            <button type="button" onClick={() => setGameOpen(false)} className="scorm-button-secondary min-h-9 px-3 text-[10px] font-semibold inline-flex items-center gap-2 shrink-0">
              <X size={13} /> Back to Free Games
            </button>
          </div>
