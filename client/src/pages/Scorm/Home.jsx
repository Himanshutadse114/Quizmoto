...
export default function ScormHome() {
  ...
  return (
    <div className="p-4 md:p-7 lg:p-8 max-w-[1440px] mx-auto">
      ...
      {error && (
        <div className="scorm-alert scorm-alert-danger mb-5">{error}</div>
      )}
      {!aiEnabled && !error && (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 mb-6">
        <StatCard label="Courses" value={courses.length} icon={BookOpen} tone="violet" />
        <StatCard label="Packages" value={packages.length} icon={Package} tone="cyan" />
        <StatCard label="Learners" value={overview.learners || 0} icon={Users} tone="neutral" />
        <StatCard label="In progress" value={overview.inProgress || 0} icon={Clock3} tone="amber" />
        <StatCard label="Completed" value={overview.completed || 0} icon={CheckCircle2} tone="green" />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.15fr_.85fr] gap-5">
        <section className="scorm-panel overflow-hidden">
          <div className="scorm-panel-header flex items-center justify-between gap-4">
            ...
            <Link to="/scorm/courses" className="scorm-text-link flex items-center gap-1.5">
            ...
            {courses.slice(0, 6).map((course) => {
              ...
              return (
                <Link
                  ...
                  className="scorm-course-row grid grid-cols-[1fr_auto] md:grid-cols-[1fr_90px_155px_auto] gap-4 items-center"
                >
                  ...
                  <div className="hidden md:block">
                    <div className="font-semibold text-sm">{stats.learners || 0}</div>
                    <div className="scorm-meta mt-1">Learners</div>
                  </div>
                  <div className="hidden md:block">
                    <div className="flex items-center justify-between gap-2 text-[10px] text-[#667085] mb-1.5">
                      <span>Progress</span><span className="font-semibold text-[#344054]">{progress.toFixed(0)}%</span>
                    </div>
                    <div className="scorm-progress-track">
                      <div className="scorm-progress-fill" style={{ width: `${Math.max(0, Math.min(100, progress))}%` }} />
                    </div>
                  </div>
        ...
        <div className="space-y-5">
          <section className="scorm-progress-hero min-h-[270px] flex flex-col justify-between">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="scorm-progress-kicker">Learner pulse</div>
                <h3 className="text-[22px] mt-1">Overall progress</h3>
              </div>
              <div className="scorm-progress-icon"><Activity size={18} /></div>
            </div>
            <div>
              <div className="scorm-progress-number">{averageProgress.toFixed(0)}%</div>
              <div className="mt-2 text-xs text-white/70">Average completion across active learner sessions.</div>
              <div className="scorm-progress-track is-dark mt-5">
                <div className="scorm-progress-fill is-light" style={{ width: `${Math.max(0, Math.min(100, averageProgress))}%` }} />
              </div>
            </div>
          </section>
          ...
          <section className="scorm-panel overflow-hidden">
            ...
            <div className="scorm-list">
              {recentLearners.length === 0 && <div className="p-6 text-center text-[#667085] text-xs">No learner activity yet.</div>}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
        <Link to="/scorm/author" className="scorm-action-card scorm-action-violet">
          <div className="scorm-action-icon"><Sparkles size={17} /></div>
          <div className="font-semibold text-[15px] mt-5">AI Author</div>
          <div className="text-xs text-[#667085] mt-1.5 leading-relaxed">Turn policy documents into structured learning while keeping editorial control.</div>
        ...
        <Link to="/scorm/reports" className="scorm-action-card scorm-action-green">
          <div className="scorm-action-icon"><BarChart3 size={17} /></div>
          <div className="font-semibold text-[15px] mt-5">Reports</div>
          <div className="text-xs text-[#667085] mt-1.5 leading-relaxed">Review completion, progress and learner performance without raw-data digging.</div>
          <div className="scorm-action-arrow"><ArrowUpRight size={15} /></div>
        </Link>
      </div>
    </div>
