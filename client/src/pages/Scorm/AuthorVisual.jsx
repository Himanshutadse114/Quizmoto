...
function LiveVisualPreview({ slide, theme }) {
  ...
  if (layout === 'hub') {
    return (
      <div className="relative w-full h-full flex flex-col items-center justify-center gap-4 p-4">
        <div className="w-24 h-24 rounded-full flex items-center justify-center text-center shadow-2xl border-2 border-white/20" style={{ background: `radial-gradient(circle at 30% 25%,${theme.accent},transparent 50%),linear-gradient(145deg,#3a3a34,#1a1a18)` }}>
          <div>
            <div className="text-[8px] uppercase tracking-wider text-white/50 mb-0.5">{slide.visualMetaphor}</div>
            <div className="text-[11px] font-bold text-white leading-tight px-2 line-clamp-2">{title}</div>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2 w-full max-w-[240px]">
          {points.slice(0, 4).map((p, i) => (
            <div key={i} className="rounded-lg border border-white/15 bg-white/10 px-2 py-1.5 flex items-center gap-1.5">
              <span className="w-5 h-5 rounded-md bg-white/20 text-white text-[9px] font-black flex items-center justify-center shrink-0">{i + 1}</span>
              <span className="text-[9px] text-white/80 font-semibold leading-snug line-clamp-2">{p}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="w-full h-full flex flex-col justify-center gap-2 p-4">
        {points.slice(0, 4).map((p, i) => (
          <div key={i} className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-full bg-white/15 border border-white/25 text-white text-[11px] font-black flex items-center justify-center shrink-0">{i + 1}</div>
            <div className="flex-1 rounded-lg border border-white/15 bg-white/10 px-2.5 py-1.5">
              <div className="text-[10px] text-white/85 font-semibold leading-snug line-clamp-2">{p}</div>
            </div>
          </div>
        ))}
        {points.length === 0 && <div className="text-center text-white/40 text-xs">Add key points to preview the process flow</div>}
      </div>
    );
  }
    ...
    return (
      <div className="w-full h-full grid grid-cols-2 gap-2 p-4">
        <div className="rounded-xl border border-emerald-400/30 bg-emerald-500/10 p-2.5">
          <div className="text-[8px] uppercase tracking-wider text-emerald-300 font-bold mb-2">Recommended</div>
          {points.slice(0, half).map((p, i) => (
            <div key={i} className="text-[9px] text-white/80 mb-1.5 leading-snug flex gap-1">
              <span className="text-emerald-400">✓</span><span className="line-clamp-2">{p}</span>
            </div>
          ))}
        </div>
        <div className="rounded-xl border border-rose-400/30 bg-rose-500/10 p-2.5">
          <div className="text-[8px] uppercase tracking-wider text-rose-300 font-bold mb-2">Watch out</div>
          {(points.slice(half).length ? points.slice(half) : ['Pause and verify']).map((p, i) => (
            <div key={i} className="text-[9px] text-white/80 mb-1.5 leading-snug flex gap-1">
              <span className="text-rose-400">!</span><span className="line-clamp-2">{p}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  if (layout === 'spotlight') {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center p-6 text-center">
        <div className="w-24 h-24 rounded-full flex items-center justify-center shadow-2xl border-2 border-white/20 mb-3" style={{ background: `radial-gradient(circle at 30% 25%,${theme.accent},transparent 50%),linear-gradient(145deg,#3a3a34,#1a1a18)` }}>
          <span className="text-3xl text-white font-black">!</span>
        </div>
        <div className="text-white font-bold text-sm mb-1 line-clamp-2">{title}</div>
        <div className="text-white/50 text-[10px] max-w-[180px] line-clamp-2">{points[0] || 'Key signal'}</div>
      </div>
    );
  }
  ...
  if (layout === 'matrix') {
    ...
    return (
      <div className="w-full h-full grid grid-cols-2 gap-2 p-4">
        {colors.map((color, i) => (
          <div key={i} className="rounded-xl border p-2.5 min-h-[70px]" style={{ borderColor: `${color}66`, background: 'rgba(255,255,255,.06)' }}>
            <div className="text-[8px] uppercase tracking-wider font-bold mb-1" style={{ color }}>{i === 0 ? 'Lower' : i === 3 ? 'Higher' : 'Watch'}</div>
            <div className="text-[9px] text-white/75 font-semibold leading-snug line-clamp-3">{points[i] || 'Risk signal'}</div>
          </div>
        ))}
      </div>
    );
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center p-4 gap-3">
      <div className="w-16 h-16 rounded-2xl border-2 border-white/20 flex items-center justify-center mb-1" style={{ background: 'rgba(255,255,255,.08)' }}>
        <span className="text-white/90 text-2xl font-black">{metaphor}</span>
      </div>
      <div className="text-white/90 text-xs font-bold text-center line-clamp-1">{title}</div>
      <div className="grid grid-cols-2 gap-1.5 w-full max-w-[240px]">
        {points.slice(0, 4).map((p, i) => (
          <div key={i} className="rounded-lg border border-white/15 bg-white/10 px-2 py-1.5 flex items-start gap-1.5">
            <span className="w-4 h-4 rounded bg-white/20 text-white text-[8px] font-black flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>
            <span className="text-[9px] text-white/80 font-semibold leading-snug line-clamp-2">{p}</span>
          </div>
        ))}
      </div>
      {points.length === 0 && <div className="text-white/40 text-[10px]">Add key points to preview cards</div>}
    </div>
  );
}
...
function LivePptCanvas({ slide, index, total, courseTitle, onChange }) {
  ...
  return (
    <div className="w-full">
      <div className="relative mx-auto rounded-[18px] overflow-hidden shadow-[0_20px_60px_rgba(40,40,36,.12)] border" style={{ background: theme.bg, borderColor: theme.accent, maxWidth: 980, minHeight: 520 }}>
        <div className="h-12 px-5 flex items-center gap-3 border-b" style={{ borderColor: theme.accent, background: 'rgba(231,231,228,.96)' }}>
          ...
          <div className="text-[10px] font-bold uppercase tracking-wider" style={{ color: theme.body }}>Part {index + 1} of {total}</div>
        ...
        <div className="grid md:grid-cols-[1.15fr_.85fr] gap-0 min-h-[440px]">
          <div className="p-7 md:p-9 flex flex-col justify-center">
            <div className="text-[10px] font-black uppercase tracking-[.12em] mb-2" style={{ color: theme.body }}>Section {index + 1} · {slide.layout || 'concept'}</div>
            <input value={slide.title || ''} onChange={(e) => onChange({ title: e.target.value, visualTitle: e.target.value })} className="w-full bg-transparent border-0 outline-none font-black leading-[1.05] tracking-tight mb-4" style={{ color: theme.primary, fontSize: 'clamp(28px, 3.2vw, 42px)' }} placeholder="Slide title — click to edit" />
            <textarea value={slide.introText || slide.content || ''} onChange={(e) => onChange({ introText: e.target.value, content: e.target.value })} rows={4} className="w-full bg-transparent border-0 outline-none resize-none leading-relaxed text-[15px]" style={{ color: theme.body }} placeholder="Learner-facing explanation — edit directly on the slide" />
            <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-2">
              {points.map((point, i) => (
                <div key={i} className="rounded-lg px-3 py-2.5 border" style={{ background: 'rgba(255,255,255,.35)', borderColor: theme.accent }}>
                  <div className="text-[9px] font-black uppercase tracking-wider mb-1" style={{ color: theme.body }}>{String(i + 1).padStart(2, '0')}</div>
                  <input value={point} onChange={(e) => { const next = [...points]; next[i] = e.target.value; onChange({ keyPoints: next }); }} className="w-full bg-transparent border-0 outline-none text-[13px] font-semibold" style={{ color: theme.primary }} placeholder={`Point ${i + 1}`} />
                </div>
              ))}
              {points.length < 6 && (
                <button type="button" onClick={() => onChange({ keyPoints: [...points, ''] })} className="rounded-lg px-3 py-2.5 border border-dashed text-[12px] font-semibold" style={{ borderColor: theme.accent, color: theme.body }}>+ Add point</button>
              )}
            </div>
            {issues.length > 0 && <div className="mt-4 text-[11px] leading-relaxed" style={{ color: '#8B4C3E' }}>{issues[0]}</div>}
          </div>

          <div className="relative min-h-[280px] md:min-h-full flex flex-col" style={{ background: theme.primary }}>
            <div className="absolute inset-0 opacity-20" style={{ background: `radial-gradient(circle at 70% 20%,${theme.accent},transparent 55%)` }} />
            <div className="relative z-10 flex-1 min-h-[200px]"><LiveVisualPreview slide={slide} theme={theme} /></div>
            <div className="relative z-10 px-3 pb-3 pt-1 flex gap-2 justify-center flex-wrap border-t border-white/10">
              <select value={slide.layout || 'cards'} onChange={(e) => onChange({ layout: e.target.value })} className="text-[10px] rounded-md px-2 py-1 bg-white/10 text-white border border-white/20" title="Layout">{COURSE_LAYOUTS.map(([id, label]) => <option key={id} value={id} className="text-black">{label}</option>)}</select>
              <select value={slide.visualMetaphor || 'shield'} onChange={(e) => onChange({ visualMetaphor: e.target.value })} className="text-[10px] rounded-md px-2 py-1 bg-white/10 text-white border border-white/20" title="Metaphor">{METAPHORS.map(([id, label]) => <option key={id} value={id} className="text-black">{label}</option>)}</select>
              <select value={slide.screenType || 'concept'} onChange={(e) => onChange({ screenType: e.target.value })} className="text-[10px] rounded-md px-2 py-1 bg-white/10 text-white border border-white/20" title="Screen type">{SCREEN_TYPES.map(([id, label]) => <option key={id} value={id} className="text-black">{label}</option>)}</select>
            </div>
          </div>
        </div>
      ...
      <p className="text-center text-[11px] text-slate-500 mt-3">Edit title, body and points directly on the slide — same look learners see. Layout/metaphor controls live on the visual panel.</p>
    </div>
  );
}
