...
function StatusBadge({ status }) {
  ...
  if (status.verified) return <span className="rounded-full border border-emerald-500/25 bg-emerald-500/10 px-2.5 py-1 text-[9px] font-semibold text-emerald-400">SMTP verified</span>;
  if (status.configured) return <span className="rounded-full border border-amber-500/25 bg-amber-500/10 px-2.5 py-1 text-[9px] font-semibold text-amber-400">SMTP connection failed</span>;
  return <span className="rounded-full border border-rose-500/25 bg-rose-500/10 px-2.5 py-1 text-[9px] font-semibold text-rose-400">SMTP not configured</span>;
}
...
export default function SuperAdminMailPanel() {
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      ...
      setNotice({ type: 'error', text: payload.message || 'Could not verify the SMTP connection.' });
    } finally {
      setChecking(false);
    }
  }, [headers]);
  ...
  const sendTest = async (kind) => {
    ...
    try {
      ...
      const label = kind === 'campaign' ? 'Campaign test email' : 'SMTP test email';
      setNotice({
        type: 'success',
        text: `${label} sent to ${response.data.recipient}.${response.data.messageId ? ` Message ID: ${response.data.messageId}` : ''}`
      });
      await checkStatus();
    } catch (error) {
      const payload = error.response?.data || {};
      if (payload.diagnostics) setStatus((current) => ({ ...(current || {}), diagnostics: payload.diagnostics, configured: payload.code !== 'MAIL_NOT_CONFIGURED', verified: false }));
    ...
    <section className="scorm-panel rounded-2xl border p-4 md:p-5 mb-4">
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div>
              ...
              <p className="mt-1 text-[10px] opacity-55">Super Admin SMTP test for OTPs, tenant invitations, course assignments and campaign emails.</p>
            </div>
        ...
        <button type="button" onClick={checkStatus} disabled={checking} className="scorm-button-secondary min-h-9 px-3 text-[10px] font-semibold inline-flex items-center justify-center gap-2 disabled:opacity-50">
          <RefreshCw size={13} className={checking ? 'animate-spin' : ''} /> {checking ? 'Checking…' : 'Check SMTP'}
        </button>
      </div>

      <div className="mt-4 grid sm:grid-cols-2 xl:grid-cols-4 gap-2.5">
        <div className="rounded-xl border px-3 py-2.5"><div className="text-[9px] uppercase tracking-[.07em] opacity-45">SMTP host</div><div className="mt-1 text-xs font-semibold break-all">{d.host || 'Not available'}</div></div>
        <div className="rounded-xl border px-3 py-2.5"><div className="text-[9px] uppercase tracking-[.07em] opacity-45">Port / security</div><div className="mt-1 text-xs font-semibold">{d.port || '—'} {d.port ? `· ${d.secure ? 'SSL/TLS' : 'STARTTLS/plain'}` : ''}</div></div>
        ...
        <div className="rounded-xl border px-3 py-2.5"><div className="text-[9px] uppercase tracking-[.07em] opacity-45">Credentials</div><div className="mt-1 text-xs font-semibold">{d.userSet && d.passwordSet ? 'Username + password set' : 'Missing credentials'}</div></div>
      </div>
      ...
      <div className="mt-4 rounded-xl border p-3.5 bg-[rgba(79,201,191,.025)]">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <button type="button" onClick={() => sendTest('smtp')} disabled={Boolean(sending)} className="scorm-button-primary min-h-9 px-3 text-[10px] font-semibold inline-flex items-center gap-2 disabled:opacity-50">
            <Send size={13} /> {sending === 'smtp' ? 'Sending…' : 'Send SMTP test'}
          </button>
          <button type="button" onClick={() => sendTest('campaign')} disabled={Boolean(sending)} className="scorm-button-secondary min-h-9 px-3 text-[10px] font-semibold inline-flex items-center gap-2 disabled:opacity-50">
            <Mail size={13} /> {sending === 'campaign' ? 'Sending…' : 'Send campaign test'}
          </button>
        </div>
        <p className="mt-2 text-[10px] leading-relaxed opacity-55">The campaign test uses the same campaign invitation template and includes a sample Email + access code passkey. If this test fails, live campaign invitations will fail too.</p>
      </div>
