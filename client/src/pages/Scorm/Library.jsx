    ...
    try {
      ...
      setMsg(
        res.data.status === 'processing'
          ? 'Upload complete. The package is being validated in the background.'
          : `Package ${res.data.status}${res.data.errorMessage ? ` · ${res.data.errorMessage}` : ''}`
      );
      setTitle('');
      setSelectedFile(null);
      await load();
  ...
  const createCourse = async (packageId, defaultTitle) => {
    try {
      const res = await axios.post(
        ...
        { packageId, title: defaultTitle || 'New course' },
        { headers }
      );
        ...
        <div className="p-4 md:p-5 grid lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)_auto] gap-3 items-end">
          <label className="block">
            ...
            <input
              ...
              className="scorm-course-search mt-1.5 w-full px-3 py-2.5 text-sm"
            />
          </label>
          ...
          <button
            ...
            className="scorm-button-primary min-h-[42px] px-4 text-xs font-semibold inline-flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <FileUp size={15} /> {uploading ? 'Uploading…' : 'Upload ZIP'}
          </button>
        </div>
      </section>
      ...
      <section className="scorm-course-list-shell rounded-xl overflow-hidden border">
        <div className="scorm-course-toolbar p-4 md:p-5 border-b flex flex-col xl:flex-row gap-3 xl:items-center justify-between">
          ...
          <div className="flex flex-col md:flex-row gap-3 md:items-center flex-1 xl:justify-end">
            ...
            <select
              ...
              className="scorm-course-search min-w-[170px] px-3 py-2.5 text-xs"
            >
        ...
        <div className="scorm-course-rows divide-y">
          ...
          {filtered.map((p) => {
            ...
            return (
              <div key={p.id} className="scorm-course-row px-5 md:px-6 py-5 transition-colors">
                <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_130px_150px_auto] gap-4 xl:items-center">
                  ...
                  <div className="flex flex-wrap gap-2 xl:justify-end">
                    {p.status === 'ready' && (
                      <button onClick={() => createCourse(p.id, p.title)} className="scorm-button-primary px-3 py-2 text-[10px] font-semibold inline-flex items-center gap-1.5">
                        <Plus size={13} /> Create course
                      </button>
                    )}
                    {(p.status === 'ready' || p.storageKeyZip) && (
                      <button onClick={() => downloadPkg(p.id, p.title)} className="scorm-button-secondary px-3 py-2 text-[10px] font-semibold inline-flex items-center gap-1.5">
                    ...
                    <button
                      onClick={() => editPkg(p)}
                      disabled={!generated}
                      title={generated ? 'Edit generated package' : 'Only generated packages can be edited'}
                      className="scorm-button-secondary px-3 py-2 text-[10px] font-semibold inline-flex items-center gap-1.5 disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                      <Pencil size={13} /> Edit
                    </button>
                    <button
