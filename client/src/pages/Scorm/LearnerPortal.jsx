...
export default function LearnerPortal() {
  ...
  const launchFlipbook = async (campaign, book) => {
    ...
    try {
      const response = await axios.post(apiUrl(`/api/scorm-learner/campaigns/${campaign.id}/flipbooks/${book.assignmentId}/launch`), {}, { headers: { Authorization: `Bearer ${learnerToken}` } });
  ...
  const launchVideo = async (campaign, video) => {
    setBusy(true); setError('');
    try {
      const response = await axios.post(apiUrl(`/api/scorm-learner/campaigns/${campaign.id}/videos/${video.videoId}/launch`), {}, { headers: { Authorization: `Bearer ${learnerToken}` } });
  ...
  const progress = allItems.length
    ? Math.round(allItems.reduce((sum, item) => sum + courseProgress(item), 0) / allItems.length)
    : 0;
          ...
          <div className="max-w-lg mx-auto">
            <section className="bg-white border border-[#dce8e5] rounded-[24px] shadow-[0_20px_60px_rgba(16,35,33,.08)] overflow-hidden">
              <div className="p-6 md:p-8 border-b border-[#e1ece9] bg-gradient-to-br from-[#e9f8f5] to-white">
                ...
                <p className="text-sm leading-relaxed text-[#617572] mt-3">Sign in with the method your administrator configured. Only identities with active assigned learning can enter.</p>
              </div>
          ...
          <>
            <section className="rounded-[24px] bg-[#102321] text-white p-6 md:p-8 overflow-hidden relative">
              ...
              <div className="relative grid md:grid-cols-[1fr_auto] gap-7 md:items-end">
                <div><div className="text-[10px] uppercase tracking-[.16em] font-bold text-[#8bd8d1]">{dashboard.workspace?.name || 'Learning dashboard'}</div><h1 className="text-3xl md:text-[46px] font-semibold tracking-[-.045em] mt-2">Welcome, {dashboard.learner?.name || 'Learner'}</h1><p className="text-sm text-white/65 mt-3 max-w-2xl">Your campaign learning, direct trackable courses, Publica publications and videos are collected here.</p></div>
            ...
            {courses.length === 0 && campaigns.length === 0 ? (
              <div className="mt-5 bg-white border border-[#dce8e5] rounded-2xl p-10 text-center"><BookOpen size={25} className="mx-auto text-[#78908c]" /><div className="font-semibold mt-3">No learning assigned</div><p className="text-xs text-[#718682] mt-1">Contact your administrator if you expected to see learning here.</p></div>
            ) : courses.length > 0 ? (
