...
/**
 * Redirects into the same-origin backend player page.
 * The SCO must find window.API on a same-origin parent; a React app on another
 * domain cannot expose the LMS API into a cross-origin content iframe.
 */
export default function ScormPlayerShell() {
  ...
  useEffect(() => {
    ...
    // Full navigation to backend origin — API + content share the same host
    window.location.replace(apiUrl(`/api/scorm/play/${registrationId}?${q.toString()}`));
  ...
  return (
    <div className="min-h-screen flex items-center justify-center text-white/50 text-sm relative z-10">
      Opening SCORM player…
    </div>
  );
}
