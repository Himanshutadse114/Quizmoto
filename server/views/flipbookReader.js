...
<div class="shell">
  ...
  <footer class="bottombar">
    ...
    <div class="status" id="status">Cover</div>
    <button class="btn" id="nextBtn"><span>Open</span> →</button>
...
function updateControls(index){
  ...
  if(first)statusEl.textContent='Cover';
  else if(last)statusEl.textContent='Back cover';
  else statusEl.textContent='Page '+(currentIndex+1)+' of '+DATA.pageCount;
  ...
  nextBtn.innerHTML=first?'<span>Open</span> →':'<span>Next</span> →';
  prevBtn.innerHTML=last?'← <span>Open</span>':'← <span>Previous</span>';
...
function fallback(){
  ...
  let index=0;
  const draw=()=>{bookEl.innerHTML='<div class="fallback"><img src="'+DATA.pages[index]+'" alt="Page '+(index+1)+'"><div>Page flip library could not load. Use Previous and Next.</div></div>';updateControls(index);fitBookFrame(index,false)};
...
function initFlipbook(){
  ...
  pageFlip.on('init',e=>{
    ...
    fitBookFrame(index,true);
  });
  pageFlip.on('flip',e=>{
    updateControls(e.data);
    playPageTurnSound();
    fitBookFrame(Number(e.data),true);
    hideHint();
  });
