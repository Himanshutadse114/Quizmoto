...
async function pruneUserAssets(r){
 const storage=getObjectStorage(),man=uManifest(r),referenced=new Set(images(r.htmlContent).map(parseAssetUrl).filter(x=>x&&x.scope==='user').map(x=>x.id)),keep=[],remove=[];
