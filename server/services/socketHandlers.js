  ...
  io.on('connection', (socket) => {
          ...
          try {
            ...
            if (player) {
              ...
              player = await Player.create({
                ...
                socketId: socket.id, sessionId: session.id, score: 0, avatar: avatar || '🛡️'
              });
            }
          } catch (dbErr) {
    ...
    socket.on('end_question', async (payload) => {
      ...
      try {
        ...
        await handleEndQuestion(pin, { hostId, ackSocket: socket });
      } catch (err) {
    ...
    socket.on('end_game', async (payload) => {
      ...
      try {
        ...
        io.to(pin).emit('game_finished', { players, teamStandings, analytics: {} });
      } catch (err) { console.error(err); }
    });
    ...
    socket.on('leave_session', async (payload) => {
      ...
      try {
        if (role === 'player') {
          ...
          let session = await GameSession.findOne({ where: { pin }, include: [{ model: Player, as: 'players' }] });
          if (!session) return;
          ...
          const payloadOut = {
            ...
            reason: 'left', temporary: false, players: session.players
          };
          ...
          try { socket.leave(pin); socket.leave('host_' + pin); } catch (_) {}
          socket.data = {};
        }
      } catch (err) {
    ...
    socket.on('send_reaction', (payload) => {
      ...
      const pin = String(rawPin).trim();
      io.to(pin).emit('new_reaction', { emoji, id: socket.id + '_' + Date.now() });
    });
      ...
      if (role === 'player' && pin) {
        try {
          ...
          if (player) {
            ...
            const session = await GameSession.findOne({ where: { pin }, include: [{ model: Player, as: 'players' }] });
            if (session) {
              io.to(pin).emit('player_left', { nickname: player.nickname, reason: 'disconnect', temporary: true, players: session.players });
              io.to(pin).emit('player_joined', session.players);
            }
          }
        } catch (err) {
