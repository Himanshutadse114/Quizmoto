...
<style>
${fontFaceCss}
:root{--background:${theme.background};--surface:${theme.surface};--primary:${theme.primary};--secondary:${theme.secondary};--text:${theme.text};--muted:${theme.muted};--primary-text:${theme.primaryText};--panel-width:clamp(240px,22vw,310px);--slide-ratio:${slideRatio};--line:#d8e8e5;--stage:#062f2d;--stage-inner:#021918;--ok:#15803d;--ok-bg:#dcfce7;--bad:#b42318;--bad-bg:#fee4e2}
...
button{font:inherit}
#app{height:100%;padding:clamp(12px,2.4vw,28px);display:grid;grid-template-columns:minmax(0,1fr) var(--panel-width);grid-template-areas:"stage controls";gap:clamp(14px,2vw,24px);background:linear-gradient(150deg,#f9fdfc 0%,#e7f4f1 100%)}
...
.course-rail{grid-area:controls;position:relative;z-index:2;min-width:0;min-height:0;padding:clamp(18px,2vw,26px);display:flex;flex-direction:column;gap:20px;background:color-mix(in srgb,var(--surface) 94%,transparent);border:1px solid var(--line);border-radius:20px;box-shadow:0 24px 60px rgba(18,60,56,.10);overflow:auto}
.rail-heading{min-width:0}
.rail-logo{display:block;width:auto;height:auto;max-width:82%;max-height:68px;margin:0 auto 18px;object-fit:contain;object-position:center}
...
.title{margin:0;display:-webkit-box;overflow:hidden;-webkit-box-orient:vertical;-webkit-line-clamp:5;font-size:clamp(1.28rem,2.15vw,1.95rem);line-height:1.1;font-weight:850;color:var(--text)}
...
html:fullscreen #app,html:-webkit-full-screen #app{padding:max(10px,env(safe-area-inset-top)) max(10px,env(safe-area-inset-right)) max(10px,env(safe-area-inset-bottom)) max(10px,env(safe-area-inset-left));grid-template-columns:minmax(0,1fr) var(--panel-width);background:linear-gradient(150deg,#f9fdfc 0%,#e7f4f1 100%)}
