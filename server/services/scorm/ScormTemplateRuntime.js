...
(function(){
  ...
  function mobileFlow(){var root=document.documentElement,w=Math.min(window.innerWidth||9999,(window.visualViewport&&window.visualViewport.width)||9999);return root.classList.contains('qmx-mobile-layout-v7')||w<=1024;}
  function fit(slide){if(!slide||!slide.classList.contains('active'))return;var n=node(slide);if(!n)return;n.style.transform='';slide.classList.remove('qmx-stage-tight','qmx-stage-critical');if(mobileFlow()){slide.style.setProperty('--qmx-stage-width','100%');slide.style.setProperty('--qmx-stage-height','auto');slide.setAttribute('data-qmx-fit-scale','1.0000');return;}applyStageGeometry(slide);var r=ratio(slide,n);if(r<.94){slide.classList.add('qmx-stage-tight');r=ratio(slide,n);}if(r<.84){slide.classList.add('qmx-stage-critical');r=ratio(slide,n);}r=Math.min(1,Math.max(.1,r));if(r<.999)n.style.transform='scale('+r.toFixed(4)+')';slide.setAttribute('data-qmx-fit-scale',r.toFixed(4));}
  ...
  function hotspot(slide){var list=cards(slide),copy=slide.querySelector('.qmx-copy');if(!list.length||!copy)return;slide.classList.add('qmx-guided-explore-ready');var grid=make('div','qmx-explore-grid'),panel=make('div','qmx-hotspot-panel'),label=make('div','qmx-hotspot-panel-label','Inspect the key points'),detail=make('p','','Choose a point to review its guidance.');panel.appendChild(label);panel.appendChild(detail);list.slice(0,4).forEach(function(c,i){var button=make('button','qmx-explore-option');button.type='button';button.setAttribute('data-qmx-explore-visited','false');button.setAttribute('aria-pressed','false');button.appendChild(make('span','qmx-explore-index',String(i+1).padStart(2,'0')));button.appendChild(make('span','qmx-explore-label',shortLabel(cardText(c))));button.onclick=function(){Array.prototype.forEach.call(grid.querySelectorAll('.qmx-explore-option'),function(x){x.classList.remove('is-selected');x.setAttribute('aria-pressed','false');});button.classList.add('is-selected');button.setAttribute('aria-pressed','true');button.setAttribute('data-qmx-explore-visited','true');label.textContent='Point '+String(i+1).padStart(2,'0');detail.textContent=cardText(c);setTimeout(fitActive,0);};grid.appendChild(button);});copy.appendChild(grid);copy.appendChild(panel);}
  ...
  function install(){document.documentElement.classList.add('qmx-template-stage-root');if(!document.body)return;document.body.setAttribute('data-qmx-course-template',TEMPLATE);Array.prototype.forEach.call(document.querySelectorAll('.slide'),decorate);fitActive();addEventListener('resize',fitActive,{passive:true});if(TEMPLATE!=='highly-interactive'){document.addEventListener('click',function(){setTimeout(fitActive,0);setTimeout(fitActive,220)},true);}var observer=new MutationObserver(function(mutations){if(TEMPLATE!=='highly-interactive'){if(mutations.length)fitActive();return;}var slideChanged=mutations.some(function(mutation){return mutation.type==='attributes'&&mutation.attributeName==='class'&&mutation.target&&mutation.target.classList&&mutation.target.classList.contains('slide');});if(slideChanged)fitActive();});observer.observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['class','aria-expanded','data-qmx-revealed','data-qmx-explore-visited']});[40,160,420,900].forEach(function(ms){setTimeout(fitActive,ms);});}
