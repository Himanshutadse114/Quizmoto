...
footer{min-height:58px;padding:10px clamp(16px,3vw,40px);display:flex;align-items:center;justify-content:space-between;gap:16px;border-top:1px solid var(--line);background:#fff}.status{font-size:10px;color:var(--muted)}.status strong{color:var(--teal-dark)}.restart{border:1px solid #b9d8d3;background:var(--soft);color:var(--ink);border-radius:10px;padding:9px 14px;font-size:10px;font-weight:800;cursor:pointer}.restart:hover,.restart:focus-visible{background:#dff3ef;outline:2px solid #45c5bc66;outline-offset:2px}
...
<div class="shell">
  ...
  <main>
    <div class="stage">
      <section class="video-card" aria-label="Course video"><div class="video-wrap"><video id="video" controls playsinline preload="metadata" controlslist="nodownload noremoteplayback" disablepictureinpicture><source id="video-source" /></video></div></section>
  ...
  <footer><div class="status" id="status">Press play when you are ready. <strong>Your progress is saved automatically.</strong></div><button class="restart" id="restart" type="button">Restart video</button></footer>
</div>
<script>
(function(){
  var BOOT=${boot};
  var video=document.getElementById('video'),source=document.getElementById('video-source');
  var ranges=[],lastVideoTime=0,lastWall=0,sessionStart=Date.now(),commitTimer=null,completed=false,playCount=0,seekCount=0,seeking=false;
  ...
  function render(){var p=progress(),position=video.currentTime||0;byId('percent').textContent=Math.round(p)+'%';byId('fill').style.width=p+'%';byId('coverage').textContent=Math.round(p)+'%';byId('position').textContent=clock(position)+' / '+clock(duration());var status=completed?'Completed':p>0?'In progress':'Not started';byId('course-status').textContent=status;byId('status').innerHTML=completed?'Course completed. <strong>Your result has been saved.</strong>':'Your position and genuine watched coverage are saved automatically.'}
  function state(){return {v:1,videoPosition:Number(video.currentTime||0),videoDuration:duration(),ranges:ranges,quizmotoProgress:progress(),videoActiveSeconds:watched(),playCount:playCount,seekCount:seekCount,completed:completed}}
  ...
  video.addEventListener('loadedmetadata',function(){var resume=Math.min(Math.max(0,Number(video.dataset.resume)||0),Math.max(0,duration()-1));if(resume>0)video.currentTime=resume;lastVideoTime=video.currentTime||0;lastWall=performance.now();render()});
  video.addEventListener('play',function(){playCount+=1;lastVideoTime=video.currentTime||0;lastWall=performance.now();render()});
  video.addEventListener('timeupdate',addPlayback);video.addEventListener('seeking',function(){seeking=true;seekCount+=1});video.addEventListener('seeked',function(){seeking=false;lastVideoTime=video.currentTime||0;lastWall=performance.now();commit(false)});video.addEventListener('pause',function(){addPlayback();commit(false)});
  video.addEventListener('ended',function(){addPlayback();completed=progress()>=90;if(completed){ranges=merge(ranges.concat([[Math.max(0,duration()-.75),duration()]]))}commit(false)});
