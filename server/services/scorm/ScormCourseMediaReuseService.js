...
async function reuseExistingCourseMedia({ pkg, analysis, storage, onProgress }) {
    ...
    if (!previousAnalysis) {
        ...
        if (content) {
            previousAnalysis = parseAnalysis(await content.async('string'));
        }
    }
    if (!previousAnalysis) {
    ...
    if (!coverPath) {
        throw mediaReuseError('The existing course cover image is missing. Rebuild was stopped to avoid generating new images automatically.');
    }
    ...
    for (const path of paths) {
        ...
        if (!entry) {
            throw mediaReuseError(`Existing visual ${path} is missing from the course ZIP. Rebuild was stopped instead of generating a replacement image.`);
        }
        files.push({
            path,
