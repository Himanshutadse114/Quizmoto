...
function escapeXml(value) {
    // Split entity strings so XML/HTML transport cannot corrupt the replacements.
    const amp = '&' + 'amp;';
    const lt = '&' + 'lt;';
    const gt = '&' + 'gt;';
    const quot = '&' + 'quot;';
...
function sceneArtwork(spec, p, mobile) {
    ...
    if (scene === 'network-attack' || scene === 'data-leak') {
        const nodes = [[0.2, 0.3], [0.5, 0.15], [0.8, 0.28], [0.3, 0.65], [0.6, 0.55], [0.85, 0.7]];
    ...
    if (scene === 'deepfake') {
      ...
      ${Array.from({ length: 24 }, (_, i) => {
            const a = 0.2 + Math.abs(Math.sin(i * 0.7)) * 0.7;
            const bh = 50 * a;
            return `<rect x="${cx - 200 + i * 18}" y="${cy + 195 - bh / 2}" width="10" height="${bh}" rx="4" fill="${p.teal}"/>`;
        }).join('')}`;
    }
    return `<circle cx="${cx}" cy="${cy}" r="220" fill="${p.teal}" opacity=".12"/>
      ...
      ${[0, 1, 2, 3, 4, 5].map((i) => {
        ...
        return `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="28" fill="${i % 2 ? p.yellow : p.teal}" stroke="${p.ink}" stroke-width="3"/>`;
    }).join('')}`;
}
