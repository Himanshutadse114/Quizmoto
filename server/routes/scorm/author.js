...
const { buildScormPackageZip } = require('../../services/scorm/ScormVisualThemeFinalizer');
...
router.post('/analyze', auth, async (req, res) => {
    try {
        ...
        const analysis = await analyzePolicy({ fileBase64: raw, mimeType: mimeType || 'application/pdf', detailLevel: detailLevel || 'detailed' });
...
router.post('/generate', auth, async (req, res) => {
    try {
        ...
        const zipBuf = await buildScormPackageZip(analysis, { templateId: Number(templateId) || 1, logoDataUrl: logoDataUrl || null });
        ...
        await pkg.save();
        try { await unpackPackage(pkg.id); } catch (e) { logger.error('scorm_ai_unpack_failed', { module: 'scorm', packageId: pkg.id, error: e.message }); }
        await pkg.reload();
        res.status(201).json({ ok: true, packageId: pkg.id, status: pkg.status, entryHref: pkg.entryHref, standard: pkg.standard, title: pkg.title, errorMessage: pkg.errorMessage });
    } catch (err) {
