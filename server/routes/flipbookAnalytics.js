...
(()=>{
  ...
  function elapsed(){return sessionStartedAt?Math.max(0,Math.floor((Date.now()-sessionStartedAt)/1000)):0}
  async function send(events,keepalive=false){
    if(!sessionToken||!events?.length)return;
    try{await fetch(BASE+'/session/'+encodeURIComponent(sessionToken)+'/events',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({events}),keepalive})}catch(_){}
  }
  function trackFlip(index){
    const next=Math.max(0,Number(index)||0);
    const direction=next>=lastIndex?'forward':'backward';
    lastIndex=next;
    const events=[{eventType:'flip',pageIndex:next,direction,elapsedSeconds:elapsed()}];
    if(typeof DATA!=='undefined'&&DATA.pageCount&&next===DATA.pageCount-1)events.push({eventType:'complete',pageIndex:next,elapsedSeconds:elapsed()});
    send(events);
  }
  function attachFlipTracking(){
    ...
    try{
      ...
      if(share)share.addEventListener('click',()=>send([{eventType:'share',pageIndex:lastIndex,elapsedSeconds:elapsed()}]));
      return true;
    }catch(_){return false}
  }
  function startHeartbeat(){
    if(heartbeatTimer)clearInterval(heartbeatTimer);
    heartbeatTimer=setInterval(()=>{if(document.visibilityState==='visible')send([{eventType:'heartbeat',pageIndex:lastIndex,elapsedSeconds:elapsed()}])},15000);
  }
  form?.addEventListener('submit',async event=>{
    ...
    try{
      const response=await fetch(BASE+'/session',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email,name,source:'share',referrer:document.referrer||''})});
      const data=await response.json().catch(()=>({}));
      if(!response.ok)throw new Error(data.message||'Could not start the reader session.');
      sessionToken=data.sessionToken;sessionStartedAt=Date.now();
  ...
  window.addEventListener('pagehide',()=>{if(sessionToken)send([{eventType:'heartbeat',pageIndex:lastIndex,elapsedSeconds:elapsed()}],true)});
})();
</script>`
    };
}

// Inject the reader identity gate and event tracker into the existing mature
// page-flip viewer without replacing its page-turn implementation.
...
// The legacy viewer still posts a one-time view event. Reader sessions now own
// authoritative view counting, so consume the old call without double-counting.
