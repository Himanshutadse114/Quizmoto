...
const SessionAuthorizationService = require('../services/SessionAuthorizationService');
...
describe('SessionAuthorizationService', () => {
    ...
    it('should return null if no host token is provided', () => {
        const hostId = SessionAuthorizationService.verifyHostToken(null);
        expect(hostId).to.be.null;
    });

    it('should generate a valid player token', () => {
        const token = SessionAuthorizationService.generatePlayerToken(10, 5, 'PlayerOne');
    ...
    it('should return null if no player token is provided', () => {
        const decoded = SessionAuthorizationService.verifyPlayerToken(undefined);
        expect(decoded).to.be.null;
    });
});
