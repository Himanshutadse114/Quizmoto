...
describe('ScormPresentationPackageBuilder', () => {
    ...
    it('packages slide images, a Quizmoto-themed quiz and complete SCORM tracking', async () => {
        ...
        expect(content.playerFont).to.equal('Inter');
        expect(zip.file('assets/fonts/inter-latin-400-normal.woff2')).to.not.equal(null);
        expect(zip.file('assets/fonts/inter-latin-600-normal.woff2')).to.not.equal(null);
        expect(zip.file('assets/fonts/inter-latin-800-normal.woff2')).to.not.equal(null);
        ...
        expect(html).to.include('grid-template-rows:minmax(0,1fr) auto');
        ...
        expect(html).to.include('width:auto;height:100%;max-width:100%;max-height:100%');
        ...
        expect(html).to.include('grid-template-columns:minmax(0,1fr)');
        expect(html).to.include('grid-template-columns:minmax(220px,.9fr) minmax(230px,1.1fr) auto');
        expect(html).to.include('id="section-label"');
        expect(html).to.include("classList.toggle('is-assessment'");
        expect(html).to.not.include('function fitRailToViewport()');
        expect(html).to.include('id="presentation"');
        expect(html).to.include('root.requestFullscreen||root.webkitRequestFullscreen');
        expect(html).to.include('root.mozRequestFullScreen||root.msRequestFullscreen');
        expect(html).to.include('document.mozFullScreenElement||document.msFullscreenElement');
        expect(html).to.include("result.catch(enterPseudoFullscreen)");
        expect(html).to.include("classList.contains('pseudo-fullscreen')");
        expect(html).to.include("document.addEventListener('MSFullscreenChange'");
